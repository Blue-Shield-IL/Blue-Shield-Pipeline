...
def get_text_classifier() -> Any:
    ...
    if TEXT_CLASSIFIER is None:
        model_name = os.getenv("FILTER_MODEL_NAME", "distilbert-base-uncased-finetuned-sst-2-english")
...
def get_zero_shot_classifier() -> Any:
    ...
    if ZERO_SHOT_CLASSIFIER is None:
        ...
        ZERO_SHOT_CLASSIFIER = pipeline("zero-shot-classification", model=model_name, device=HF_DEVICE)
...
def get_ner_pipeline() -> Any:
    ...
    if NER_PIPELINE is None:
        ...
        NER_PIPELINE = pipeline("ner", model=model_name, aggregation_strategy="simple", device=HF_DEVICE)
    return NER_PIPELINE

def filter_post(raw_post_data: dict[str, Any], threshold: float = 0.6) -> Optional[ProcessedPost]:
...
def analyze_content(processed_post: ProcessedPost) -> ProcessedPost:
    ...
    try:
        ...
        processed_post.sentiment = SENTIMENT_LABEL_MAP.get(raw_label, SENTIMENT_LABEL_MAP.get(raw_label.lower(), "Neutral"))
    except Exception:
